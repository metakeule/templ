...
import (
	...
	ttempl "github.com/metakeule/templ"
	"regexp"
	"strings"
	"testing"
)
...
var (
	...
	Replacement1 = ttempl.NewPlaceholder("replacement1")
	Replacement2 = ttempl.NewPlaceholder("replacement2")

	MapM            = map[string]string{}
	StringMapM      = map[string]string{}
	ByteMapM        = map[string][]byte{}
	PlaceholderMapM = map[*ttempl.Placeholder]string{}
	StringsM        = []string{}
)
...
func PrepareM() {
	...
	for i := 0; i < 5000; i++ {
		...
		phk := ttempl.NewPlaceholder(key)
		PlaceholderMapM[&phk] = val
	}
...
func TestReplace(t *testing.T) {
	...
	if s := tt.MustReplace(Replacement1.Set("repl1"), Replacement2.Set("repl2")).String(); s != Expected {
...
func TestReplaceN(t *testing.T) {
	...
	if s := tt.MustReplace(Replacement1.Set("repl1"), Replacement2.Set("repl2")).String(); s != ExpectedN {
		t.Errorf("unexpected result for %s: %#v", "replacer.Template", s)
	}
...
func TestReplaceM(t *testing.T) {
	...
	if s := tt.MustReplace(setters...).String(); s != ExpectedM {
		t.Errorf("unexpected result for %s: %#v", "replacer.Template", s)
	}
}
